...
class PayService {
	 ...
	 ** 实例程序如下
	 ***/
	public function pay($uid,$orderNumber,$price,$ccid){
		$price=sprintf("%.2f", $price);
		$platform="webAlipay";
		 $bankCode="0000";
		 $subject = urlencode("流量");
		 $sysCode="1002";
		  $merchantUrl = "http://boxapi.wifigo.cn/api/pay/index?ccid=".$ccid;
	 ...
	 ** 实例程序如下
	 ***/
	public function payon($uid,$orderNumber,$price,$ccid,$session_id){
		$price=sprintf("%.2f", $price);
		$platform="webAlipay";
		 $bankCode="0000";
		 $subject = urlencode("3.5GB全国流量卡");
		 $sysCode="1003";
		 $merchantUrl = $_SERVER['SERVER_NAME']."/api/payinfoss/index?ccid=".$ccid."&session_id=".$session_id;
		 $sysKey="aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d";
		 $config  = Yaf_Registry::get('config');
        $url     = $config->user->pay->url;
		 $key =  hash('sha256',$uid.$orderNumber.$price.$sysCode.$sysKey);
